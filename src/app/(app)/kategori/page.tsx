...
import { LayoutGrid, BookOpen, PlayCircle } from 'lucide-react'

export const metadata: Metadata = {
  title: 'Kategori | VirtualWork - Simulasi Kerja Virtual',
  description: 'Jelajahi 11 kategori pekerjaan: Data Analyst, Project Coordinator, Social Media Management, dan lainnya. Temukan materi dan latihan untuk setiap kategori.',
}
...
async function getCategories() {
  try {
    return await prisma.category.findMany({
      ...
      include: {
        _count: {
          select: { materis: true, latihans: true },
        },
      },
    })
  } catch (error) {
...
export default async function KategoriPage() {
  ...
  const totalMateri = categories.reduce((acc, c) => acc + c._count.materis, 0)
  const totalLatihan = categories.reduce((acc, c) => acc + c._count.latihans, 0)
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
      <div className="mb-10">
        ...
        <p className="text-muted-foreground text-lg max-w-2xl">
          Kelompokkan materi dan latihan sesuai minatmu. Pilih salah satu dari{' '}
